    ...
    namespace WebAthenPs.API.Controllers
    {
        ...
        [HttpGet("GetMembersName/{chatId}")]
        public IActionResult GetMembersName(Guid chatId)
        {
            // Primeiro, obtemos os IDs dos membros do chat
            var members = _hubService.GetMembers(chatId);

            // Em seguida, buscamos os nomes dos membros com base nos IDs
            var memberNames = members.Select(userId => _hubService.GetUserNameById(userId)).ToList();

            // Retornamos os nomes dos membros como resposta
